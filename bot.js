...
client.on("message",async message => {
  ...
  if(args[0] == "zc!uptime") {
  var link = args[1]
if(!link) return message.reply("Bir Link Girmelisin!");
const  links = wiodb.fetch("uptime");
if(!link.startsWith('https://')) return message.channel.send('Bu link degil?')
...
for(var i in x) {
  if(x[i].url.includes(link)) return message.channel.send("Link Sistemde Bulunmakta")
    ...
    message.channel.send("Link Sisteme Eklenmistir");
